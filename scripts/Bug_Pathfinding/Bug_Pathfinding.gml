						...
						if (!(check[c] == -1))//if not nothing or if not visited
						{
							...
							if !(check[c] == -4)
							{
								for(var j = 0; j < array_length(check[c].attributes); j++)
								{
									...
									{
										if ((check[c].attributes[j] ==avoid[k]) || (check[c].tier > id.tier))
										{
...
function Bug_Path_to_Coords(){
	...
	var path_next = []; //placeholder for scoping next path steps. Iteratively becomes path last
	var _max, obst, check;
	var a_max = array_length(avoid);
